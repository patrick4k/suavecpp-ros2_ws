...
void MaskingSubscriber::callback(const Vector3Msg::SharedPtr msg) {
    ...
    if (m_enable)
    {
        ...
        constexpr float MAX_DELTA_VELOCITY = 0.25*MAX_VELOCITY;
        ...
        if (std::abs(velocity.forward_m_s - m_prevVelocity->forward_m_s) > MAX_DELTA_VELOCITY)
        {
            suave_err << "Large difference in forward velocity: " << velocity.forward_m_s << " vs " << m_prevVelocity->forward_m_s << std::endl;
            this->shutdown();
            return;
        }
        if (std::abs(velocity.right_m_s - m_prevVelocity->right_m_s) > MAX_DELTA_VELOCITY)
        {
            suave_err << "Large difference in right velocity: " << velocity.right_m_s << " vs " << m_prevVelocity->right_m_s << std::endl;
            this->shutdown();
            return;
        }
        if (std::abs(velocity.down_m_s - m_prevVelocity->down_m_s) > MAX_DELTA_VELOCITY)
        {
